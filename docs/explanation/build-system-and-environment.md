...
## モダンC++を支える指針とツール（Core Guidelines + clang-tidy）

「現代にあえてC++を書く」とき、力を入れるべきは**整形スタイルではなくコーディング指針**。
コード規約には2つの層があり、"モダン"が効くのは後者。

```mermaid
flowchart TD
    subgraph L1["① 整形スタイル（見た目）"]
        CF[".clang-format<br/>LLVMベース"]
    end
    subgraph L2["② コーディング指針（設計・実装）"]
        CG["C++ Core Guidelines<br/>（モダンC++の事実上の標準）"]
        CT[".clang-tidy<br/>自動チェック"]
        CG --> CT
    end
```

- **① 整形スタイル**（インデント・括弧位置）… `.clang-format` の話。プリセット（LLVM /
  Google 等）の違いは**好みの問題**で、どれが"モダン"ということはない。一貫していれば良い。
- **② コーディング指針**（言語機能の使い方・設計）… ここが本丸。
  **C++ Core Guidelines**（Stroustrup と Sutter 主導）がモダンC++の事実上の標準。

### なぜ Core Guidelines + clang-tidy なのか

このリポジトリで学んできた「単一引数コンストラクタは `explicit`」「メンバ初期化リスト」
「継承より合成」などは、すべて Core Guidelines の精神そのもの。これを**自動でチェック**
するのが clang-tidy で、特に次の効果がある：

- `modernize-*` … 古い書き方を現代的な書き方へ直す提案
- `cppcoreguidelines-*` … Core Guidelines 違反の指摘
- 「書いた瞬間にモダンから外れていたら警告が出る」状態になり、**学習効果が高い**

整形プリセット選び（①）に悩むより、②に投資するのがモダンC++への近道、という判断。

### チェックを無効化するより、コードを合わせる

clang-tidy の指摘には「無効化する」と「コードを直して合わせる」の2つの対応がある。
このリポジトリは、**指摘に意味があるならコードを直す**方針を採る。無効化は、純粋に
スタイル寄りでノイズになるチェック（マジックナンバー等）に限る最終手段。

実際、導入時に出た指摘がそのまま設計の学びになった：

| 指摘 | 直し方（コードを合わせる） | 学び |
|------|------------------------|------|
| 状態を使わないメソッド（Calculator） | 自由関数にする | 状態を持たない演算はクラスにしない |
| メソッドが状態を使っていない（App::run） | App に状態（名前）を持たせる | 状態があるものだけクラスにする |
| main が例外を漏らしうる | main に最外 try/catch を置く | 例外を漏らさない安全網の定石 |

結果として「Calculator は状態が無いから自由関数／App は状態を持つからクラス」という、
**いつクラスを使うべきかの対比**が手元に残った。clang-tidy は単なる校正ツールではなく、
設計を学ぶ相棒になっている。
